...
//                NSLog(@"Database and tables created.");
            ...
            sqlStatement = "CREATE TABLE IF NOT EXISTS History (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, title TEXT, url TEXT, update_time DATE)";
            if (sqlite3_exec(databaseHandle, sqlStatement, NULL, NULL, &error) == SQLITE_OK) {
//                NSLog(@"Database and tables created.");
            } else {
                NSLog(@"Error: %s", error);
...
{
    NSString *queryStatement = [NSString stringWithFormat:@"SELECT update_time FROM PageCache ORDER BY update_time limit %ld, 1", (long)count];
...
- (void)insertHistory:(NSString *)title url:(NSString *)url
{
    ...
    NSString *insertStatement = @"INSERT INTO History (title, url, update_time) VALUES (?, ?, ?)";
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(databaseHandle, [insertStatement UTF8String], -1, &stmt, nil) == SQLITE_OK) {
        ...
        NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
        [dateFormat setDateFormat:@"yyyy-MM-dd HH:mm:ss Z"];
        NSString *updateTimeString = [dateFormat stringFromDate:[NSDate date]];
        sqlite3_bind_text(stmt, 3, [updateTimeString UTF8String], -1, nil);
    }
    if (sqlite3_step(stmt) == SQLITE_DONE) {
...
- (NSArray *)searchHistory:(NSString *)query limit:(NSInteger)limit
{
    ...
    NSString *queryStatement = [NSString stringWithFormat:@"SELECT * FROM History WHERE url like '%%%@%%' LIMIT %ld", query, (long)limit];
...
- (NSArray *)allHistory:(NSInteger) limit
{
    NSMutableArray *histories;
    NSString *queryStatement = [NSString stringWithFormat:@"SELECT * FROM History Order By id DESC LIMIT %ld", (long)limit];
    ...
    if (sqlite3_prepare_v2(databaseHandle, [queryStatement UTF8String], -1, &statement, NULL) == SQLITE_OK) {
        while (sqlite3_step(statement) == SQLITE_ROW) {
            ...
            NSDictionary *history = [NSDictionary dictionaryWithObjectsAndKeys:title, @"title", url, @"url", date, @"update_time", nil];
            if (histories == nil) {
                histories = [[NSMutableArray alloc] init];
            }
            [histories addObject:history];
